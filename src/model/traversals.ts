...
import { Scalar, TypeExpression, ListType, NonNullType, Product, Sum, TypeParameter, FunctionType, ProductField, Argument } from './typeExpression'
...
export function traverseTypeExpression(te: TypeExpression, mapper: TraverseMapper): TypeExpression {
  if (te instanceof IDRefLocator) {
    ...
    return mapper.onListType ? mapper.onListType(te) : new ListType({
      of: traverseTypeExpression(te.of, mapper)
    })
  } else if (te instanceof NonNullType) {
    return mapper.onNonNullType ? mapper.onNonNullType(te) : new NonNullType({
      of: traverseTypeExpression(te.of, mapper)
    })
  } else if (te instanceof Product) {
    return mapper.onProduct ? mapper.onProduct(te) : new Product({
    ...
    return mapper.onSum ? mapper.onSum(te) : new Sum({
    ...
    return mapper.onFunctionType ? mapper.onFunctionType(te) : new FunctionType({
      arguments:
        te.arguments.map((argument: Argument) => {
          return new Argument({
            id: argument.id,
            name: argument.name,
...
export async function traverseTypeExpressionAsync(te: TypeExpression, mapper: TraverseMapperAsync): Promise<TypeExpression> {
  if (te instanceof IDRefLocator) {
    return mapper.onIDRefLocator ? mapper.onIDRefLocator(te) : te
  } else if (te instanceof LocalNameLocator) {
    return mapper.onLocalNameLocator ? mapper.onLocalNameLocator(te) : te
    ...
    return mapper.onProduct ? mapper.onProduct(te) : new Product({
      extendable: te.extendable,
      fields:
        await Promise.all(te.fields.map(async field => {
    ...
    return mapper.onSum ? mapper.onSum(te) : new Sum({
      variants: await Promise.all(te.variants.map(variant => traverseTypeExpressionAsync(variant, mapper)))
    })
  } else if (te instanceof TypeParameter) {
    return mapper.onTypeParameter ? mapper.onTypeParameter(te) : te
  } else if (te instanceof FunctionType) {
    return mapper.onFunctionType ? mapper.onFunctionType(te) : new FunctionType({
      arguments:
        await Promise.all(te.arguments.map(async (argument: Argument) => {
          return new Argument({
            id: argument.id,
            name: argument.name,
            description: argument.description,
